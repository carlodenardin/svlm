...
from src.core.run_tests import run_bulk_test
from src.data.problem_loader import get_available_problems
from src.utils.const import MODEL_MAP, DIAGRAMS_LEVELS, DIAGRAMS_TYPES, DIAGRAM_MAP, PROMPT
from src.utils.utils import get_css_style, get_gpu_info, run_python_code, update_link
    ...
    with gr.Blocks(css = get_css_style()) as demo:
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("On The Fly"):
                ...
                run_button.click(run_python_code, inputs = code_input, outputs = code_output)
            ...
            with gr.Tab("Bulk Test"):
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        start_bulk_test_button = gr.Button("Start Tests", variant = "primary")
                        
                gr.Markdown("---")
                bulk_output = gr.Textbox(label = "Output", lines = 10)
